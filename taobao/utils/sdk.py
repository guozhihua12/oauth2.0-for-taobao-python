...
'''
Python client SDK for taobao API using OAuth 2.
'''
...
class APIClient(object):
    ...
    '''
    def __init__(self, app_key, app_secret, redirect_uri=None, response_type='code', domain='oauth.taobao.com', version='2'):
    ...
    def get_authorize_url(self, redirect_uri=None, **kw):
        ...
        params = _encode_params(client_id=self.client_id, response_type=response_type, redirect_uri=redirect, **kw)
        return '%s?%s' % (self.auth_url, params)

    def request_access_token(self, code, redirect_uri=None, grant_type='authorization_code'):
        '''
        ...
        if remind_in:
            ...
            if rtime < expires:
                expires = rtime
        return JsonDict(access_token=r.access_token, expires=expires, expires_in=expires, open_uid=r.get('open_uid', None), taobao_user_nick=r.get('taobao_user_nick', None))

    def request_revoke(self, access_token):
        try:
            # r = _http_get('%s%s' % (self.auth_url, 'revokeoauth2'), access_token=access_token)
            r = _http_get(self.auth_url, access_token=access_token)
            return r.result
        except APIError, e:
            return True
